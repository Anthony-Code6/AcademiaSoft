...
AS
BEGIN
	SELECT C.id AS IDCARGO, C.IDAULA AS IDCAULA_CARGO, C.IDCURSO AS IDCURSO_CARGO,
		   C.IDPROFESOR AS IDPROFESOR_CARGO, C.IDPERIODO AS IDPERIODO_CARGO,
		   CONCAT(A.descripcion, ' - ', CU.DESCRIPCION) AS DESCRIPCION, year(p.fectrimestre1) AS ANIO
