...
--SELECT A.ID,A.CODIGO,A.DESCRIPCION, COUNT(A.CODIGO) AS CANTIDAD FROM AULA AS A INNER JOIN MATRICULA AS M 
--ON A.ID=M.IDAULA INNER JOIN CARGO_PROFESOR AS C
--ON A.ID=C.IDAULA
--WHERE A.ID=289
...
/*CREATE PROCEDURE SP_USUARIO_VALIDOS
AS
BEGIN
    SELECT U.id, U.codigo, U.email, U.password, U.tipo
    FROM USUARIO AS U
    LEFT JOIN ALUMNO AS A ON U.id = A.idusuario
    LEFT JOIN PROFESOR AS P ON P.idusuario = U.id
    WHERE A.idusuario IS NULL AND P.idusuario IS NULL;
END
GO*/
...
-- EXEC SP_LISTAR_NOTA 5, 7, 1, 2023
/*
...
/*
	...
	SELECT M.*
	FROM MATRICULA AS M INNER JOIN CARGO_PROFESOR AS C ON C.idaula = M.idaula
	WHERE 
	VALUES (@IDMATRICULA, @IDCARGO);
	select * from matricula

	SELECT * FROM CARGO_PROFESOR ORDER BY IDPERIODO, IDAULA
	SELECT CONCAT(FORMAT(sysdatetime(),'yyyy') -2,FORMAT(sysdatetime(),'-MM-dd'))
	SELECT A.CODIGO, A.APELLIDO, A.NOMBRE, B.NOTA1, B.NOTA2, B.NOTA3, B.NOTA4, B.PROMEDIO
	FROM ALUMNO AS A
	INNER JOIN MATRICULA AS M ON A.id = M.idalumno
	LEFT JOIN BOLETA_NOTA AS B ON M.id = B.idmatricula
	--INNER JOIN PERIODO AS P ON M.idperiodo = P.id
